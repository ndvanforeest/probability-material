...
\begin{solution}
 ...
 % We can consider both ranges separately for $b \in [0,\alpha]$ and $b \in (\alpha, \inf)$. Then in the first case the co\"efficient of $b^2$ is positive, so we should consider the boundary condition, $b=\alpha$ is the maximizer. In the second case $b$ is negative, so we should make it as small as possible, i.e., choose $b = lim_{x \downarrow \alpha} x$. 
...
\begin{solution}
  \begin{enumerate}
    ...
    %Take $i=b-1$. Then solve for $\alpha$ in $\alpha(b-1) = \alpha(b-2)/2+1/2$, because $p_{b}=1$. This gives $\alpha=1/b$.
    \item $ p_0 = p_1 / 2 = (p_2/2)/2 = (2\alpha)/4 = 1/2b$ 
  \end{enumerate}
  
%Where does $\alpha i$ come from? The equation in $3$ is a difference equation, a quite simple one, it is linear and it has constant coefficients. As you will learn in later courses, the general solution can be written as $\beta + \alpha i$ where $\beta$ and $\alpha$ are constants. As its characteristic root is $1$, with multiplicity 2. One of the initial value's p_b = 1 is known, it seems intuitive that the probability is not determined by a constant component, but only by how far away from $b$ the drunk is. 
