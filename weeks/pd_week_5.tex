...
We should check that with this result we can retrieve $\E X = 1/\lambda$. For this we need an intermediate result.
\begin{exercise}
...
\begin{exercise}
Use the above two exercises, Adam's law, and LOTP to show that $\E{X} = \E{\E{X\given Y}} = 1/\lambda$, where $Y=\1{X\leq \tau}$.
