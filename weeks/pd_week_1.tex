...
\item BH Book: Read Section 6.6,  sums of independent rvs with MGFs
\item BH video: 18
\end{enumerate}
...
Next, let \(X_{i}\in\{1, 2, \ldots, n\}\) be the choosen day of patient \(i\).
Then \(Y_{i}(j)=\1{X_{i}=j}\) is 1 if \(X_i=j\) and 0 else.
Thus, when we have \(m\) patients, \(S_{m}=\sum_{i=1}^{m} Y_i(j)\) is the number of jobs that arrived on day \(j\). In the above queueing model, there are 5 arrivals per day, hence \(m=5n\).
...
Exercise BH.6.22
\begin{solution}
See our solution manual for BH problems.
\end{solution}
\end{exercise}
...
\begin{exercise}
Read BH.6.22. Relate that exercise to the story of the patients of Lecture 1.
\begin{solution}
...
Exercise BH.6.13
\begin{solution}
See the sample solutions of BH.
\end{solution}
\end{exercise}
