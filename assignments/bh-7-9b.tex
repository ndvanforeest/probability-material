...
\begin{exercise}
  In Python (or R), devise a monte carlo test using the \texttt{min()} function to find if \texttt{scipy.stats.geom} (or the \texttt{\_geom()} functions in R) uses the ``geometric'' or ``first succes'' distribution.
